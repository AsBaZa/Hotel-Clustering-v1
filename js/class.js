...
class ClusterPlot extends PlotPlotly{
  ...
  paint(dataRaw = null){
    ...
    Plotly.newPlot(this.element, this.data,
      {
        title: 'Clustering with Hotel/Pension price time-series',
        yaxis: {
          title: "Price €",
          rangemode: "tozero"
        },
...
class CategoryPlot extends PlotPlotly{
  ...
  paint(){
    ...
    Plotly.newPlot(this.element, this.data,
      {
        title: 'Hotel Distribution by Category',
        yaxis: {
          title: this.select1.value === "denak" ? "Total Number of Hotels" :
                                                  "Percentage % of Hotels",
          rangemode: "tozero"
        },
        barmode: this.select1.value === "denak" ? 'stack' : 'group',
        margin: {
          b: 30,
          t: 30,
          l: 45,
...
class StratumPlot extends PlotPlotly{
  ...
  paint(){
    ...
    Plotly.newPlot(this.element, this.data,
      {
        title: 'Hotel Distribution by Stratum',
        yaxis: {
          title: this.select1.value === "denak" ? "Total Number of Hotels" :
                                                  "Percentage % of Hotels",
          rangemode: "tozero"
    ...
    this.shpfile = new L.Shapefile('shapefile/basque_est.zip', {
      onEachFeature: function(feature, layer) {
        if (feature.properties) {
          ...
          layer.bindPopup(Object.keys(feature.properties).map(function(x) {
            return estratoak[feature.properties[x]];
          }).join(""), {
            maxHeight: 200,
            autoPan: false,
            keepInView: true
